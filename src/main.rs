...
pub fn count_tiles_by_suit_rank(
...
) -> HashMap<TileSuit, HashMap<TileRank, u32>> {
    ...
    for tile in tiles.iter() {
        let inner_map: &mut HashMap<TileRank, u32> = tile_counts_by_suit
            .entry(tile.suit())
            .or_insert_with(HashMap::new);
        let mut rank = tile.rank();
        if ignore_red_fives && rank == TileRank::Number(NumberTileRank::RedFive) {
            rank = TileRank::Number(NumberTileRank::Five);
        }
        let rank = rank;
...
fn first_copy_index(
    ...
    for tile_idx in 0..tiles.len() {
        ...
        if tile.suit() == tile_suit {
            ...
            } else if ignore_red_fives && tile.rank() == TileRank::Number(NumberTileRank::RedFive) {
                return Some(tile_idx);
            }
        }
    }
...
mod tests {
    ...
    fn test_hand_grouping_1() {
        ...
        let hand_groups: Vec<TileGroup> = Vec::new();
    ...
    fn test_hand_grouping_4() {
        ...
        let winning_tiles = Vec::from([
            ...
            Tile::from_string("8m"),
            Tile::from_string("8m"),
            Tile::from_string("5z"), // white dragon
            Tile::from_string("5z"),
            Tile::from_string("5z"),
            Tile::from_string("2m"),
        ]);

        let hand_groups: Vec<TileGroup> = vec![
            TileGroup::Sequence {
                open: true,
                tiles: [
                    Tile::from_string("4s"),
    ...
    fn test_hand_grouping_8() {
        ...
        let winning_tiles = Vec::from([
            ...
            Tile::from_string("2p"),
            Tile::from_string("3p"),
            Tile::from_string("5p"), // TODO is this supposed to be a 4p tile?
            Tile::from_string("3s"),
            Tile::from_string("0s"),
            Tile::from_string("4s"),
        ]);
